describe("Bookings Page", () => {
  ...
  beforeEach(() => {
    ...
    describe("Booking admin", () =>{
        ...
        it("should display User updated successfully when edit a user", () => {
            cy.intercept('PATCH', '/user/2', (req) => {
                req.headers['Authorization'] = `Bearer ${fakeTokenAdmin}`;
                req.reply({
                    statusCode: 201,
                    message: "Ok",
            ...
            cy.get('input[name="firstname"]').clear().type("Lana");
            cy.get('input[name="lastname"]').clear().type("RHOADES");
            
            cy.contains("Save").click();
            
            cy.wait('@editUser');
            
            cy.get(".MuiAlert-root").should("contain", "User updated successfully");
        });

        it("should display User deleted successfully when delete a booking", () => {

            cy.intercept('DELETE', '/user/2', (req) => {
                req.headers['Authorization'] = `Bearer ${fakeTokenAdmin}`;
                req.reply({
                    statusCode: 200,
                    body: { id: '2'},
            ...
            cy.get(".MuiAlert-root").should("contain", "User deleted successfully");
        });
    })
});