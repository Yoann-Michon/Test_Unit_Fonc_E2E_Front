...
export const bookingService = {
  ...
  async getAllBookings() {
    try {
      ...
      const url = userRole === 'admin'
        ...
        : `${import.meta.env.VITE_BACK_API_URL}/booking/user/${decodedToken.sub}`;
  ...
  async searchBookings(query: string) {
    try {
      ...
      const url = userRole === 'admin'
        ...
        : `${import.meta.env.VITE_BACK_API_URL}/booking/search/${query}/user/${decodedToken.id}`;
  ...
  async createBooking(bookingData: { hotelId: string; checkInDate: string; checkOutDate: string }) {
    try {
      const token = AuthService.getToken();
      if (!token) {
      ...
      const response = await fetch(`${import.meta.env.VITE_BACK_API_URL}/booking`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  async updateBooking(id: string, updateData: { checkInDate?: string; checkOutDate?: string }) {
    try {
      const token = AuthService.getToken();
      if (!token) {
        throw new Error('User not authenticated');
      ...
      const userBookingUrl = `${import.meta.env.VITE_BACK_API_URL}/booking/${id}`;
